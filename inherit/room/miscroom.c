...
void init()
{
	...
	set("item_desc/sign",
        ...
        起步价：一两白银。
        当前保管费用资费状态：
        普通玩家：保存价值"+MONEY_D->money_str(10000)+"的物品收费"+MONEY_D->money_str(get_value2(86400,to_float(10000)))+"/天。
        贵宾玩家：保存价值"+MONEY_D->money_str(10000)+"的物品收费"+MONEY_D->money_str(get_value2(86400,to_float(10000))/2)+"/天。
        	\n",
        );
...
int get_value(object me,mapping list)
{
	...
	if(me->query("registered")==3) savetime = savetime / 2;
