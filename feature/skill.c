	...
	if (learned[skill] > (spi = (skills[skill] + 1) * (skills[skill] + 1))) {
		...
		if (weak_mode > 1) {
			...
			tell_object(this_object(), HIC "你的「" + to_chinese(skill) + "」进步了！\n" NOR);
                   //天赋“博学天才”影响效果:  by lsxk@hsbbs  2008/3/14
                   if((int)this_object()->query("relife/study/bxtc",1) > 0){
                       if(random(100) < ((int)this_object()->query("relife/study/bxtc",1)*2)){
                           skills[skill]++;
                           tell_object(this_object(),HIC"你只觉脑中豁然开朗，对你的「" + to_chinese(skill) + "」理解又进了一步！\n"NOR);
                           }
                   }
