...
int main(object me, string arg)
{
	...
	if (me->query("prepare_retire")) {
		if( time()- me->query("pk_time") < 604800 )
		{
			me->delete("prepare_retire");
		...
		if( (time()-me->query("prepare_retire_time")) < 604800 )
			return notify_fail("你金盆洗手的时间还未到，你可以在"+CHINESE_D->chinese_time(604800-time()+me->query("prepare_retire_time"))+"后正式金盆洗手。\n");
		...
		CHANNEL_D->do_channel(me,"chat",HIY"我已厌倦无尽的江湖争斗，决定在七日之后金盆洗手，从此不再过问江湖纷争！"NOR);
...
如果你决定金盆洗手，在第一次输入此命令后，将有七天时间了结
江湖恩怨，在此期间，如果发生主动PK，将视为你放弃金盆洗手。
七天后再次输入本命令，将正式退出江湖。

如果在七天内你决定放弃金盆洗手，请输入“retire abandon”命令。
如果你放弃，在之后的七天里，你将不能再次金盆洗手。

如果你在最近的一个礼拜内参与了江湖争斗那将不能金盆洗手，同样
你在准备金盆洗手的一个礼拜内参与了江湖争斗，那金盆洗手就会被
取消。
