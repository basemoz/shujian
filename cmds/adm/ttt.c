...
// add for temp title display
	...
	if (base_name(ob) == "/clone/user/user") {
		line += sprintf("┃职业：%-4s%-18s叛师：%-18s犯    规：%-12s┃\n",
			ob->query_user_type() != "worker"?HBCYN+HIY"武士":HBCYN+HIY"工匠",
			my["no_pk"]?"「已金盆洗手」"NOR:""NOR,
			my["betrayer"]?HIY+chinese_number(my["betrayer"])+"次"NOR:"无",
			my["rules"]?HIR+chinese_number(my["rules"])+"次"NOR:"无",
		);

		line += sprintf("┃杀人：%-22s玩家：%-18s正当防卫：%-12s┃\n",
	...
	if (my["mud_age"] >= 86400)
		line += sprintf(" 平均每天在线：%s\n",
			CHINESE_D->chinese_time(to_int(my["mud_time"]*400.0/(time()-my["birthday"])*216))
		);
	write(line);
	return 1;
}
