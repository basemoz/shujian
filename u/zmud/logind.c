...
private void get_email(string, object,int,int,int,int);
...
//	}
	...
	if (!ppl && !wiz_level(arg)) {
		...
		usr2 = filter_array(children(USER_OB),(: userp :));
			    for(i=0; i<sizeof(usr2); i++) {
         if (ip==query_ip_number(usr2[i])) ip_cnt++;
        }
         if (ip_cnt>8)
         {
...
//hxsd为运行修改，删除
		if (__DIR__"sited"->is_log(arg, ip)) {
			destruct(ob);
			return;
		}
	}
...
private void get_passwd(string pass, object ob)
{
	...
	switch (ob->query("registered")) {
		...
		default:
         ...
         if (!ob->query("confirm_passwd")){
		 ob->set_temp("confirm_passwd", crypt(pass, 0));
	}
	}
//hxsd修改
/*
...
private void select_gift(string arg, object user)
{
	int str,intl,con,dex;
	
	if(sscanf(arg,"%d %d %d %d",str,intl,con,dex)!=4){
	  printf(HIY"\n请输入您想要的属性，四项之和必须等于八十点，每一项不能少于十点，不能大于三十点：\n"NOR);  			
	  input_to((:select_gift:), user);
	  return;
	}
	...
	printf("\n你确定吗？(Yes/No)\n");
	input_to("yes_no", user,str,intl,con,dex);
	return;
}

private void yes_no(string arg,object user,int str,int intl,int con,int dex)
{
	...
	if(arg=="No"||arg=="no"||arg=="N"||arg=="n"){
	  ...
	  printf("\n你确定吗？(Yes/No)\n");
	  input_to((:yes_no:), user);
	  return;
	}
	get_email("guest", user,str,intl,con,dex);
	return;
}

private void get_email(string email, object ob,int str,int intl,int con,int dex)
{
	object user;
	...
	user->set("food", 200);
		if(!ob->query("confirm_passwd")){
  ob->set("confirm_passwd", ob->query_temp("confirm_passwd"));
	user->set("password", ob->query_temp("confirm_passwd"));
	user->set("confirm_passwd", ob->query_temp("confirm_passwd"));
	}
	user->set("water", 200);
	user->set("env/wimpy", 50);
	user->set("enter_wuguan", 1);
		user->set("registered", 3);
                	user->set("vip/vip_time",time());
                	user->add("vip/vip_time",6*24*3600);
                  user->set("vip/vip_start_time",time());
...
varargs void enter_world(object ob, object user, int silent)
{
	...
	if (!silent && !wiz_level(user->query("id")))
		if (user->query("registered") < 2) write(""+read_file(MOTD));
