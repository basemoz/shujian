...
void create()
{
    ...
    set("inquiry", ([
        ...
        if (clonep()) {
                ob = unew("/u/yueying/jinshe/obj/jszf-book");
                if (!ob) ob = new("/clone/money/gold");
                ob->move(this_object());
                sb = unew("/u/yueying/jinshe/obj/gift");
...
int ask_xia1()
{
    ...
    if (me->query("env/text")){
        call_out("do_happen1",6,me);
    return 1;}
        if(me->query("quest/jinshe2/step1",1))
    {
         call_out("do_happen1",6,me);
     return 1;}
...
int accept_object(object who, object ob)
{
    ...
    object me = this_player();
        ...
        {
             command("hmm");
             command("say 你是从哪里得到的这封信?当心我去找月影和戴黛举报你!");
             msg = HIG"温仪柳眉一皱:来人啊,把这个骗子轰出去!!!。\n"NOR;
             message_vision(msg, me);
             message_vision(HIR"\n一群家丁应声而出,将$N乱棒打晕,丢了出去。\n"NOR,me);
             me->unconcious();
