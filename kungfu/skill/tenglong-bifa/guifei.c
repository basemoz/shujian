...
*/
 ...
 if(random(ap)>dp/4)
	...
	msg = HIY"\n$n闪避不及，被$N一脚踢中，面色立刻变的惨白！\n"NOR;		msg += damage_msg(damage, "瘀伤");
		//	msg = replace_string( msg, "$l", "后心");
                //	msg = replace_string( msg, "$w", "匕首");
			                	
 		message_vision(msg,me,target);	COMBAT_D->report_status(target);
if ( me->is_fighting(target)){
 message_vision( HIY"\n$N再顺势反过身来，咯咯轻笑数声，左手向着$n的头顶轻轻拂去。\n"NOR,me,target);	perform2(me, target);}	}
 		else
 		{
...
if ( me->is_fighting(target))
 ...
 message_vision(msg,me,target);
 perform2(me, target);}       	}
  /*  message_vision( MAG"\n$n被$N搂住头顶，只觉得心中一荡，神情迷糊之间却没发觉到$P右手竟已握住了"+weapon->name()+MAG"！\n"NOR,me,target);  me->set_temp("sld/pfm/guifei",1);       COMBAT_D->do_attack(me,target,0,1);
...
me->start_busy(1);
 ...
 if ( random(10)>5 
&& me->is_fighting(target))    COMBAT_D->do_attack(me,target,0,TYPE_SUPER);
...
ap = me->query_skill("tenglong-bifa",1)*me->query_dex();
 		dp = target->query_skill("dodge",1)* target->query_dex();
  if( target->is_busy() ) dp= dp/2;  if(random(ap)>dp/2)
...
weapon->unequip();
   COMBAT_D->do_attack(me,target,0,2);
    ...
    me->delete_temp("sld/pfm/guifei"); 
        me->add_temp("apply/attack",-lvl); me->add("neili",-250);
  	me->add("jingli",-130);
