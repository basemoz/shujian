...
*/
        ...
        me->add("neili", -350);
        me->add("jingli", -200);
        if(objectp(vweapon)  /* && weapon->query("sharpness")>3 */)
...
else if((int)me->query_skill("tenglong-bifa", 1)>350) { message_vision(HIY"\n$N双腿一缩，似欲跪拜，右臂却横掠而出，"+weapon->name()+HIY"急削向$n！"NOR,me,target);
	...
	if(!living(target)) pp = 1;
      ...
      me->start_perform(4, "「鲁达拔柳」");
        //   target->start_busy(2);
                ...
                if (damage > 3500) damage = 3500 + (damage-3500)/20;
if(me->query("gender")!="男性")
   damage = damage/2; 
 // 非男性威力小   
...
if( !target
 ...
 lvl = (int)me->query_skill("tenglong-bifa", 1)  /4; me->add_temp("apply/attack", lvl); 
      
weapon->unequip();
 
COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 3);
 weapon->wield(); if ( me->is_fighting(target) && random(me->query_skill("tenglong-bifa", 1)) > 250)
   COMBAT_D->do_attack(me, target, me->query_temp("weapon"), 3);
