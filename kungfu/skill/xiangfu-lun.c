...
string *wushuai_msg = ({
HIY"$N伸手搭在法轮上往斜里一拨，法轮在空中划出一道弧线，向$n飞去，势如猛牛！"NOR,
HIY"$N不待法轮落地，抢上前伸脚一挑一送，法轮陡然飞起，从不可思议的角度撞向$n"NOR,
HIY"$N吐气发力，双掌齐推，法轮去势劲急，呜的一声冲向$n。"NOR,
HIY"$N双手箕张，拦在$n身前，法轮在空中旋转而回，袭向$n的后脑。"NOR,
HIY"$N霹雳般一声断喝，侧身单拳硬生生击在法轮上，法轮方向猛的一转，直追向$n。"NOR,
HIY"$N右手圈转，将法轮轻轻巧巧接在手中，紧接着手腕急翻，左手平推，法轮旋向$n的$l。"NOR,
HIY"$N凭内力将法轮定在手掌心上急旋，眼光扫向$n，法轮紧跟着飞了过去。"NOR,
HIY"$N跃入半空，双掌自上而下推出，法轮凭空加速，威势惊人。"NOR,
HIY"$N抢前几步，手腕一转，借势送出法轮，随后向$n虚拍几掌，封住了$n的退路。"NOR,
});
...
mixed hit_ob(object me, object victim,int damage)
{
	...
	if( random(10) >= 6 && random(me->query("combat_exp")) > victim->query("combat_exp")/ 3*2 ){
		message_vision(HIW"$N手臂暴长，施展「锁」字诀，"+wp1->name()+HIW"探出，嚓的一声锁住了$n"+wp2->name()+HIW"。\n"NOR, me,victim);
		if( random(me->query_str(1)) > victim->query_str(1) ) {
			me->add("neili", -150);
			message_vision(HIG+me->query("name")+HIG"运劲回拉，$N臂骨喀的一声，险些就被拉断，剧痛之下手一松，"+wp2->name()+HIG"被夺了过去！\n"NOR, victim);
			wp2->unequip();
			wp2->move(me);
		}
		else if(random(me->query_str()) > victim->query_str()/3*2){           
			me->add("neili", -50);
			message_vision(HIG+me->query("name")+HIG"运劲回拉，$N被带得踉跄几步，但觉虎口剧痛，"+wp2->name()+HIG"脱手飞出！\n"NOR, victim);
			wp2->unequip();
