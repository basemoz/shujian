...
{
return usage == "hand" || usage == "dodge" ||usage == "parry" || usage == "force";
}
...
if (!jiali) jiali=90;
        if( damage_bonus > 100 && random(me->query_skill("taixuan-gong",1))>140 
        ) {
        victim->receive_wound("qi", (random(jiali)+60));
