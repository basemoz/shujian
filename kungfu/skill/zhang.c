...
string ask_me_6()
{
		...
		if (!(fam = this_player()->query("family")) || fam["family_name"] !="武当派"){
             ...
             command("say 太极圆转之意，若非本派门人，贫道岂能轻易传授？");
             return 0;
        }

        if ( this_player()->query_temp("tjdp"))
        	return "为师不是刚给过你太极道袍么？";
        if ( present("taiji daopao", this_player()) )
                return RANK_D->query_respect(this_player()) +
                "你以为我门道袍是什么易得之物么？真是贪得无厌！";
        if ( present("taiji daopao", environment()) )
                return RANK_D->query_respect(this_player()) +
                "你以为我门道袍是什么易得之物么？真是贪得无厌！";
        ob = unew(ARMOR_D("taijipao"));
        if(!clonep(ob)) return "贫道已将太极袍给了你其他师兄弟了。";

        if(clonep(ob) && ob->violate_unique()){
           	destruct(ob);
           	return "贫道已将太极袍给了你其他师兄弟了。";
        }
        
	    this_player()->set_temp("tjdp",1);
        message_vision("\n张三丰沉声长吟道：「"HIW"太极圆转和形动，阴阳开合随吾身！"NOR"」。\n\n", this_player());
		ob->move(this_player());
		return "贫道昔年的随身之袍就内含"HIC"太极圆转"NOR"之意，你要好生自行拿去参悟。」\n\n";
