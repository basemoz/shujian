...
int perform(object me, object target)
{
	  ...
	  &&(int)me->query_skill("zixia-gong", 1) < 150)
       ...
       if(target->query("cs_upgrade") && target->query("cs_kill_id") == me->query("id") && me->query("quest/hsjf/cangsong")!="pass"){
          if(me->query("id")!="zmud"&&(!random(50)
          ||(me->query_temp("sj_credit/quest/hs/cangsong") && !random(3))
          ||me->query("id")=="zmud")){
        ...
        message_vision(HIG"\n$N手中的"+weapon->query("name")+HIG"突然一晃，满场游走，一身青衫化成了一片青影，一招「"+GRN+"苍"+
           "松迎客"+HIG+"」直刺$n。\n"NOR,me,target);
	message_vision(HIM"\n只见$N将手中"+weapon->query("name")+HIM+"缓缓刺出,剑身竟丝毫不晃动,可见这招内力运用已入化境!\n"NOR,me);

    if((random(target->query_skill("dodge"))<me->query_skill("dodge")/2&&me->query("quest/hsjf/cangsong")=="pass")
           || random(me->query("combat_exp")) > target->query("combat_exp")/2
           || target->is_busy() ){
           damage = 4 * (int)me->query_skill("huashan-jianfa", 1) + (int)me->query_skill("sword",1);
           damage = damage/2 + random(damage);
           if ( me->query("quest/hsjf/cangsong")=="pass") {
               damage = damage*2;
               message_vision(HIW "$N这招「"+GRN+"苍松迎客"+HIW+"」已得精要,此时使将出来,威力乃今非昔比!\n"NOR,me,target);
           }
           if (wizardp(me)&&me->query("env/cangsong"))
   ...
   message_vision(HIB"\n$N竟不给$n有丝毫喘息机会,紧跟着"+weapon->query("name")+HIB"再一次挥出,但听“噌”的一声,"+
        ...
        if( random(target->query_skill("dodge")) < me->query_skill("dodge")/2
           || random(me->query("combat_exp")) > target->query("combat_exp")/2
           || target->is_busy() ){
           damage = 5 * (int)me->query_skill("huashan-jianfa", 1) + (int)me->query_skill("force",1);
           damage = damage/2 + random(damage*3/2);
           if ( me->query("quest/hsjf/cangsong")=="pass") {
               damage = damage*2;
               message_vision(HIR "$N早已将此招练得纯熟万分,剑招即出,气势便犹如千军万马般,直袭$n破绽之处!\n"NOR,me,target);
           }
           if (wizardp(me)&&me->query("env/cangsong"))
               write(HIR"damage is; "+damage+" !\n"NOR);
           me->add("neili", -damage/3);
