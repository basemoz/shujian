...
/*      e = 110 * j + random(50);*/
        ...
        if(ob->query_temp("wd/wd_jobok") ){        	
            ...
            if(ob->query("bonus/武当") < 1)
                TASKREWARD_D->get_reward(ob,"武当",shen,0,0,1,0,0,0,"宋远桥");
            else
                TASKREWARD_D->get_reward(ob,"武当",shen,0,0,ob->query("bonus/武当"),0,0,0,"宋远桥");
...
string ask_me2()
{
        ...
        if(!wizardp(me)){
        if (me->query("menggu"))
          //    return "哼，你以身属蒙古，还想做什么？";                  // ..... laugh 修改错别字一个 by campsun 2004.2.16
