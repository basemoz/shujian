...
void flee(object me,int idx,int team)
{
	...
	if( g>0 ) {
		...
		if(objectp(me1)) me1->set("job_name","ÉñÁúµºº£Õ½");
	...
	if(objectp(me1)) {
		if(team==1) exp = me1->add_exp_combat(exp,"Ê©ÀÅ","´óÇå¹úº£Õ½");
		else exp = me1->add_exp_combat(exp,"Â½¸ßÐù","ÉñÁúµºº£Õ½");
	...
	if(objectp(me2)) {
		if(team==1) exp = me2->add_exp_combat(exp,"Ê©ÀÅ","´óÇå¹úº£Õ½");
		else exp = me2->add_exp_combat(exp,"Â½¸ßÐù","ÉñÁúµºº£Õ½");		
