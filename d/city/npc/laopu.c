...
int ask_sj_credit()
{
    ...
    if( i<1 ){
        ...
        command("say 哈哈，"+(string)me->query("name")+"你蝶梦楼战绩辉煌，如果愿意，你可以在我这里兑换黄金，算当奖励吧！"NOR);
        write(HIR"兑换指令："+HIC+"duihuan XXX to 黄金\n"NOR);
...
int do_duihuan(string arg)
{
   ...
   if( sscanf(arg, "%d to %s",i,str) ){
       ...
       if( i< SJ_CREDIT_RATE){
           write(HIW"你必须要花费大于"+HIM+chinese_number(SJ_CREDIT_RATE)+HIW+"点蝶梦楼积分才能兑换黄金！\n"NOR);
           return 1;
       }

       if( j< SJ_CREDIT_RATE){
           write(HBRED"你目前的蝶梦楼积分根本不够兑换一个书剑黄金。请继续努力！\n"NOR);
           return 1;
       }
       me->add("balance",i/SJ_CREDIT_RATE*10000);
       me->add("quest/special/score",-i);
       write(HIG"兑换完毕!你此次兑换花费了"+HIM+chinese_number(i)+HIG+"点蝶梦楼积分，增加了"+HIW+chinese_number(i/SJ_CREDIT_RATE*10000)+HIG+"锭黄金！\n"NOR);
       return 1;
   }
   else{
       write(HIR"\n兑换指令："+HIC+"duihuan XXX to 黄金\n"NOR);
       write(HIG"其中XXX为你要兑换掉的蝶梦楼积分数量！\n"NOR);
       write(HIY"\n请按照正确格式输入，来进行黄金兑换！\n"NOR);
       return 1;
   }
