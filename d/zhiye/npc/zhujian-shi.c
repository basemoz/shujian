...
int do_answer(string arg)
{
   ...
   if(wizardp(me) && INTERMUD_MUD_NAME!="SHTX" && INTERMUD_MUD_NAME!="SJTY") return notify_fail("你老人家不是一直用SMASH的么？\n");
	...
	if( !me->query("imbue_reward")){
                if( me->query("combat_exp") < 2000000 ) // By Spiderii 控制建立垃圾帐号打造
        ...
        if(!npc->go_dazao(lu,me,cmds)){
        	...
        	return notify_fail("出错了，快去通知zmud。\n");
        }
        command("say 稍等片刻。\n");
        return 1;
}
...
string ask_weilan()
{
	...
	if(wizardp(me)  && INTERMUD_MUD_NAME!="SHTX" && INTERMUD_MUD_NAME!="SJ" ) return "你老人家不是一直用SMASH的么？\n";
