// shizhe.c 转生使者
#include <ansi.h>
#define SKILLS_LEVEL            216
#define KNOWLEDGE_LEVEL 200
inherit NPC;
string ask_zhuansheng();
string ask_xingxiu();
string ask_sld();
string ask_wudang();
string ask_ss();
...
void create()
{
    set_name(HIR"天涯转生使者"NOR, ({ "shizhe" }));
         set("nickname", HBCYN"天涯转生使者"NOR);
    set("long", "你可以ask shizhe about 转世,在ask shizhe about 拜师,喜欢哪个门派就ask shizhe about ***,addoil all \n");
    set("gender", "男性");
    set("age", 70);
    set("per", 24);
    set("str", 40);
...
{ 
    ...
    return "恭喜您转世重修成功，请通过ask shizhe about 拜师 来获得门派技能。\n"; 
}
string ask_xingxiu()
{
    if (!this_player()->query("relife/zhuanshi")) return "您似乎不够资历吧！？\n";
    if (this_player()->query("zhuanshi/ok")) return "您似乎已经获得过技能了吧！？\n";
