...
/* 参数说明：
    ...
    hardquo:    难度系数，数值从-10到10，数值越高，说明难度高，所以奖励多。负数的时候，奖励将会小于基础奖励。-10,经验奖励为0，奖励maxexp数量的潜能。
...
//全局定义限定玩家增长的EXP上限为50M
#define LIMITEXP 50000000
...
*/
        ...
        if(obj->query("relife/exp_ext"))
            exp = exp + exp * (int)obj->query("relife/exp_ext",1) / 100;
        ...
        //通宝部分
//        if(credit < 0) credit = exp / 70 + random(exp / 100);
//        if(credit != 0) msg += HIW + chinese_number(credit) + HIC + "个" + HIW + "通宝！\n"NOR;

        //特殊部分，例如ebook等
        expquo = 0;
        if(obj->query_condition("db_exp")){  //经验之书
            if(random(3))
                expquo = random(exp);
            else expquo = exp;
