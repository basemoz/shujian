...
static string code_wizard;
static string code_code;
static int file_part;
...
void read_callback2(int fd, string str)
{
	...
	else if(sscanf(str,"EESUCCESS:%s",str)==1)
	{
		...
		else if(sscanf(str,"move%sdone",str)==1)
		{
			...
			log_file("static/super_ftp",sprintf("由于%-30s,%-8s更新%s",code_code,code_wizard,file_name),this_object());
