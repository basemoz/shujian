...
void test_robot()
{
	...
	int i = sizeof(obs), j = 10, count,k,l;
	string *arg;
	...
	while (j--) {		
		ob = obs[random(i)];
		count = ob->query("mud_time");//玩家实际连线时间
		k = uptime()-ob->query_temp("time");
		l = ob->query("combat_exp") - ob->query_temp("combat_exp");
		if (l*3600/i < 3000) continue;//如果小于3k每小时
		if (wiz_level(ob)) continue;
		if (ob->query_condition("killer")) continue;
		if (ob->is_robot()) continue; // 如果是机器人了，就不用检查了
		...
		if (ob->query("registered") >=3 && count - ob->query("robottest") < 10800) continue;
		if (ob->query("combat_exp") < 100000 && count - ob->query("robottest") < 7200) continue;
		if (ob->query("combat_exp") >= 100000 && count - ob->query("robottest") < 5400) continue;
		arg = explode(base_name(environment(ob)), "/");
		if (arg[0] == "u") continue;
		if (arg[1] == "death") continue;
		if (arg[1] == "leitai" && arg[2] != "bwdh") continue;
		if (arg[1] == "wizard") continue;
		break;
	}
	if (j < 0) return;
	ppl = ob->query("id");
	ob->set("robottest", count);
	count = 3+random(2);
	robot = new(ANTIROBOT);
	CHANNEL_D->do_channel( robot, "sys", ob->name(1)+"("+capitalize(ppl)+")正在接受机器人 "+count+" 的抽查。");
	//有奖励的
        robot->test_robot(ob,1);
}
