...
//                case "huanxi":
	...
	//并站生日纠正 	
	if( user->query("from_site") =="LN" && !user->query("birthday_modified") )
	{
		user->set("birthday_old",user->query("birthday_old")?user->query("birthday_old"):user->query("birthday"));
		user->set("birthday", time() - user->query("mud_age"));
		user->set("birthday_modified",1);
		//user->set("birthday", 1080750000 );
		tell_object(user,sprintf(HIG"由于转站经验值被CUT，现修正你的生日为%s以做补偿。\n"NOR, CHINESE_D->chinese_date((user->query("birthday") - 14*365*24*60) * 60) )  );
	}
		
	//存款纠正
	if(user->query("balance") > F_BANKER->query_balance_limit(user) )
	{
		tell_object(user, HIY "书剑京城总行通知您：因为您的存款有误，已经查核清楚，请不要担心。\n" NOR);
	...
	//某些Perform Exert 自动恢复 在这里	
	if(user->query("dali/jujing") && !user->query_temp("dali/jujing") )
	{
		//Jujing /kungfu/skill/qiantian-yiyang/jujing.c
		tell_object(user, HIY"\n时间一长，你感觉支持不住，精神开始分散了。\n" NOR);
		i = user->query("dali/jujing");
				
		user->add("max_jing",i);
		user->add("eff_jing",i);
		if( user->query("jing") > user->query("eff_jing") )
			user->set("jing",user->query("eff_jing"));
		
		if( user->query("eff_qi") > user->query("max_qi") )
			user->set("eff_qi",user->query("max_qi"));
		if( user->query("qi") > user->query("eff_qi") )
			user->set("qi",user->query("eff_qi"));
		
		user->delete_temp("dali/jujing");
		user->delete("dali/jujing");
		user->delete_temp("dali/jujing1");
	}
}
...
/*
	tick_count = (int)me->query_temp("check_gold_tick");
	if( (tick_count%5) == 0 ) {
		if( objectp(gold = present("gold_money", me)) && gold->check_gold() )
			tell_object(me,"突然从旁边窜出一个黑影，抢了你的一些黄金就跑……\n");
		if( objectp(gold = present("thousand-cash_money", me)) && gold->check_cash() )
			tell_object(me,"突然从旁边窜出一个黑影，抢了你的一些银票就跑……\n");
	}
*/	
	//vip 洗手
